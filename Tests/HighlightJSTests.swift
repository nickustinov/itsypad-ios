...
final class HighlightJSTests: XCTestCase {
    ...
    private func foregroundColor(in attrStr: NSAttributedString, at token: String, file: StaticString = #file, line: UInt = #line) -> UIColor? {
        ...
        return attrStr.attributes(at: nsRange.location, effectiveRange: nil)[.foregroundColor] as? UIColor
    }
    ...
    private func assertToken(_ token: String, in attrStr: NSAttributedString, hasColor hex: String, file: StaticString = #file, line: UInt = #line) {
        ...
        XCTAssert(
            ...
            "Color mismatch for \"\(token)\": got \(colorHex(color)), expected \(hex)",
            file: file, line: line
        )
    }
    ...
    private func assertHighlighted(_ token: String, in attrStr: NSAttributedString, file: StaticString = #file, line: UInt = #line) {
        guard let color = foregroundColor(in: attrStr, at: token, file: file, line: line) else { return }
        let defaultColor = colorFromHex(Self.defaultFg)
        var cr: CGFloat = 0, cg: CGFloat = 0, cb: CGFloat = 0, ca: CGFloat = 0
        var dr: CGFloat = 0, dg: CGFloat = 0, db: CGFloat = 0, da: CGFloat = 0
        color.getRed(&cr, green: &cg, blue: &cb, alpha: &ca)
        defaultColor.getRed(&dr, green: &dg, blue: &db, alpha: &da)
        let isSame = abs(cr - dr) < 0.02 && abs(cg - dg) < 0.02 && abs(cb - db) < 0.02
    ...
    private func colorHex(_ c: UIColor) -> String {
